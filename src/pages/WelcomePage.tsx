...
export function WelcomePage({
  ...
  /* ------------------------------------------------------------ */
  /* CANVAS ANIMATION (unchanged from your original code)         */
  /* ------------------------------------------------------------ */
  useEffect(() => {
    ...
    class WavyLine {
      ...
      update() {
        for (let i = 0; i < this.points.length; i++) {
          ...
          const normalizedDistance = Math.sqrt(Math.pow(dx / mouseRef.current.radiusX, 2) + Math.pow(dy / mouseRef.current.radiusY, 2));
      ...
      draw(ctx: CanvasRenderingContext2D) {
        ...
        if (this.index === numLines - 1) {
          ...
          gradient.addColorStop(0, `rgba(232, 114, 95)`);
          gradient.addColorStop(1, `rgba(232, 114, 95)`);
        ...
        const last = this.points[this.points.length - 1];
        ctx.lineTo(last.x + 50, last.y);
        ctx.strokeStyle = `rgba(232, 114, 95, ${opacity})`;
        ctx.lineWidth = thickness;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        ctx.stroke();
        ctx.strokeStyle = `rgba(232, 114, 95, ${opacity * 0.3})`;
  ...
  return <div className="min-h-screen relative">
    <style>{`
      ...
      /* Constant backing layer to improve blur */
      .backdrop-overlay {
        position: absolute;
        inset: 0;
      ...
      .glass {
        ...
        /* lock blur into its own GPU layer */
        will-change: transform, opacity, backdrop-filter;
        transform: translateZ(0);
      }

      /* Frosted glass grain texture */
      .glass::before {
        content: '';
        position: absolute;
        inset: 0;
    ...
    <canvas ref={canvasRef} className="fixed inset-0 w-full h-full" style={{
      zIndex: 1
    }} />

    <div className="relative z-20 min-h-screen flex items-center justify-center p-6">

      {/* backing layer behind card — improves blur strength */}
      <div className="backdrop-overlay"></div>

      <div className="w-full max-w-md relative">
        <div className="text-center mb-8 smooth-entrance">
          <h1 className="text-6xl font-bold text-deep-blue mb-3">
            Pathfinder
          </h1>
        ...
        {/* Improved glass card with frosted grain */}
        <div className="glass p-8 shadow-xl border border-white/40">
          <div className="space-y-6 smooth-entrance">
            <div className="space-y-1 mb-8">
              <h2 className="text-2xl font-semibold text-deep-blue">
                Get Started
              </h2>
              <p className="text-deep-blue/50 text-sm">
                Enter your details to begin your personalised path
              </p>
            </div>

            <Input label="Name" value={name} placeholder="Enter your name" onChange={onNameChange} required />

            <Input label="Website URL" value={url} onChange={onUrlChange} placeholder="https://yourwebsite.com" type="url" required />
